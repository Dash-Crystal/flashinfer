...
__global__ void bsfp8_reset_reused_pages_kernel(const SlotType* __restrict__ slot_mapping,
                                                uint8_t* __restrict__ page_format,
                                                float* __restrict__ page_error_stats,
                                                const int num_slots, const int page_size) {
  const int token = blockIdx.x * blockDim.x + threadIdx.x;
  if (token >= num_slots) return;
  const int64_t slot = static_cast<int64_t>(slot_mapping[token]);
  if (slot >= 0 && slot % page_size == 0) {
    const int64_t page = slot / page_size;
    page_format[page] = 0;
    const float unmeasured = __int_as_float(0x7f800000);
    page_error_stats[page * 2] = unmeasured;
    page_error_stats[page * 2 + 1] = unmeasured;
  }
}

template <typename InType, typename SlotType, int THREADS = 128>
__global__ void bsfp8_kv_quant_slots_kernel(
    const InType* __restrict__ k_input, const InType* __restrict__ v_input,
    const SlotType* __restrict__ slot_mapping, const float* __restrict__ k_global_scale_ptr,
    const float* __restrict__ v_global_scale_ptr, uint8_t* __restrict__ k_output,
    uint8_t* __restrict__ v_output, uint8_t* __restrict__ k_block_scales,
    uint8_t* __restrict__ v_block_scales, uint8_t* __restrict__ page_format,
    float* __restrict__ page_error_stats, const float* __restrict__ routing_thresholds,
    const int num_slots, const int page_size, const int num_heads, const int head_dim,
    const int64_t in_stride_page, const int64_t in_stride_token,
    const int64_t in_stride_head, const int64_t in_stride_dim,
    const int64_t out_stride_page, const int64_t out_stride_token,
    const int64_t out_stride_head, const int64_t out_stride_dim,
    const int64_t sf_stride_page, const int64_t sf_stride_token,
    const int64_t sf_stride_head, const int64_t sf_stride_dim) {
  const int token_idx = blockIdx.x;
  if (token_idx >= num_slots) return;
  const int64_t slot = static_cast<int64_t>(slot_mapping[token_idx]);
  if (slot < 0 || slot % page_size != page_size - 1) return;

  const int64_t page = slot / page_size;
  __shared__ float global_scales[2];
  if (threadIdx.x == 0) {
    global_scales[0] = *k_global_scale_ptr;
    global_scales[1] = *v_global_scale_ptr;
  }
  __syncthreads();
  ...
  float signal_sum_sq = 0.0f;
  float error_sum_sq = 0.0f;
  float signal_amax = 0.0f;
  float error_amax = 0.0f;

#pragma unroll
  for (int kv = 0; kv < 2; ++kv) {
    const InType* input = kv == 0 ? k_input : v_input;
    uint8_t* output = kv == 0 ? k_output : v_output;
    uint8_t* scales = kv == 0 ? k_block_scales : v_block_scales;
    const float global_scale = global_scales[kv];
...
#pragma unroll
      ...
      if (lane == 0) {
        ...
        scales[sf_offset] = sf_fp8.__x;
      }
      const float encode_scale =
          sf_value == 0.0f ? 0.0f : reciprocal_approximate_ftz(global_scale * sf_value);
      const int64_t output_offset = page * out_stride_page + token * out_stride_token +
                                    head * out_stride_head + dim * out_stride_dim;
      __nv_fp8_e4m3 encoded = __nv_fp8_e4m3(value * encode_scale);
      output[output_offset] = encoded.__x;
      const float reconstructed = static_cast<float>(encoded) * sf_value * global_scale;
      const float error = fabsf(reconstructed - value);
      signal_sum_sq = fmaf(value, value, signal_sum_sq);
      error_sum_sq = fmaf(error, error, error_sum_sq);
      signal_amax = fmaxf(signal_amax, fabsf(value));
      error_amax = fmaxf(error_amax, error);
    }
  }
...
#pragma unroll
  for (int offset = 16; offset > 0; offset /= 2) {
    signal_sum_sq += __shfl_down_sync(uint32_t(-1), signal_sum_sq, offset);
    error_sum_sq += __shfl_down_sync(uint32_t(-1), error_sum_sq, offset);
    signal_amax =
        fmaxf(signal_amax, __shfl_down_sync(uint32_t(-1), signal_amax, offset));
    error_amax = fmaxf(error_amax, __shfl_down_sync(uint32_t(-1), error_amax, offset));
  }
  __shared__ float warp_signal_sum_sq[THREADS / 32];
  __shared__ float warp_error_sum_sq[THREADS / 32];
  __shared__ float warp_signal_amax[THREADS / 32];
  __shared__ float warp_error_amax[THREADS / 32];
  if (warp_lane == 0) {
    warp_signal_sum_sq[warp] = signal_sum_sq;
    warp_error_sum_sq[warp] = error_sum_sq;
    warp_signal_amax[warp] = signal_amax;
    warp_error_amax[warp] = error_amax;
  }
  __syncthreads();
  if (warp == 0) {
    constexpr int NUM_WARPS = THREADS / 32;
    signal_sum_sq = warp_lane < NUM_WARPS ? warp_signal_sum_sq[warp_lane] : 0.0f;
    error_sum_sq = warp_lane < NUM_WARPS ? warp_error_sum_sq[warp_lane] : 0.0f;
    signal_amax = warp_lane < NUM_WARPS ? warp_signal_amax[warp_lane] : 0.0f;
    error_amax = warp_lane < NUM_WARPS ? warp_error_amax[warp_lane] : 0.0f;
#pragma unroll
    for (int offset = 16; offset > 0; offset /= 2) {
      signal_sum_sq += __shfl_down_sync(uint32_t(-1), signal_sum_sq, offset);
      error_sum_sq += __shfl_down_sync(uint32_t(-1), error_sum_sq, offset);
      signal_amax =
          fmaxf(signal_amax, __shfl_down_sync(uint32_t(-1), signal_amax, offset));
      error_amax =
          fmaxf(error_amax, __shfl_down_sync(uint32_t(-1), error_amax, offset));
    }
    if (warp_lane == 0) {
      const float relative_rms =
          signal_sum_sq == 0.0f ? 0.0f : sqrtf(error_sum_sq / signal_sum_sq);
      const float relative_amax = signal_amax == 0.0f ? 0.0f : error_amax / signal_amax;
      page_error_stats[page * 2] = relative_rms;
      page_error_stats[page * 2 + 1] = relative_amax;
      page_format[page] = relative_rms <= routing_thresholds[0] &&
                                  relative_amax <= routing_thresholds[1]
                              ? 1
                              : 0;
    }
  }
...
void bsfp8_kv_quant_slots(TensorView k_input, TensorView v_input, TensorView slot_mapping,
                          TensorView k_global_scale, TensorView v_global_scale,
                          TensorView k_output, TensorView v_output, TensorView k_block_scales,
                          TensorView v_block_scales, TensorView page_format,
                          TensorView page_error_stats, TensorView routing_thresholds) {
  CHECK_CUDA(k_input);
  CHECK_CUDA(v_input);
  ...
  CHECK_CUDA(k_output);
  CHECK_CUDA(v_output);
  CHECK_CUDA(k_block_scales);
  CHECK_CUDA(v_block_scales);
  CHECK_CUDA(page_format);
  CHECK_CUDA(page_error_stats);
  ...
  TVM_FFI_ICHECK(head_dim % BSFP8_BLOCK_SIZE == 0)
      << "head dimension must be divisible by " << BSFP8_BLOCK_SIZE;
  TVM_FFI_ICHECK(k_output.ndim() == 4 && v_output.ndim() == 4 &&
                 k_output.size(0) == num_pages && k_output.size(1) == page_size &&
                 k_output.size(2) == num_heads && k_output.size(3) == head_dim &&
                 v_output.size(0) == num_pages && v_output.size(1) == page_size &&
                 v_output.size(2) == num_heads && v_output.size(3) == head_dim)
      << "payload outputs must match the input shape";
  TVM_FFI_ICHECK(k_block_scales.ndim() == 4 && v_block_scales.ndim() == 4 &&
                 k_block_scales.size(0) == num_pages &&
                 k_block_scales.size(1) == page_size &&
                 k_block_scales.size(2) == num_heads &&
                 k_block_scales.size(3) == head_dim / BSFP8_BLOCK_SIZE &&
                 v_block_scales.size(0) == num_pages &&
                 v_block_scales.size(1) == page_size &&
                 v_block_scales.size(2) == num_heads &&
                 v_block_scales.size(3) == head_dim / BSFP8_BLOCK_SIZE)
      << "block-scale outputs have the wrong shape";
  TVM_FFI_ICHECK(slot_mapping.ndim() == 1) << "slot_mapping must be 1D";
  TVM_FFI_ICHECK(page_format.ndim() == 1 && page_format.size(0) == num_pages)
      << "page_format must have one byte per physical page";
  TVM_FFI_ICHECK(page_error_stats.ndim() == 2 && page_error_stats.size(0) == num_pages &&
                 page_error_stats.size(1) == 2 && page_error_stats.dtype() == dl_float32)
      << "page_error_stats must be float32 [num_pages, 2]";
  TVM_FFI_ICHECK(routing_thresholds.ndim() == 1 && routing_thresholds.size(0) == 2 &&
                 routing_thresholds.dtype() == dl_float32)
      << "routing_thresholds must be float32 [2]";
  TVM_FFI_ICHECK(k_global_scale.numel() == 1 && v_global_scale.numel() == 1)
      << "global scales must be scalar tensors";

  ffi::CUDADeviceGuard device_guard(k_input.device().device_id);
  cudaStream_t stream = get_stream(k_input.device());
  const int num_slots = slot_mapping.size(0);
  constexpr int THREADS = 128;
  constexpr int RESET_THREADS = 256;
  const int reset_blocks = (num_slots + RESET_THREADS - 1) / RESET_THREADS;
  TVM_FFI_ICHECK(slot_mapping.dtype() == dl_int64)
      << "slot_mapping must use int64";
  bsfp8_reset_reused_pages_kernel<int64_t><<<reset_blocks, RESET_THREADS, 0, stream>>>(
      static_cast<const int64_t*>(slot_mapping.data_ptr()),
      static_cast<uint8_t*>(page_format.data_ptr()),
      static_cast<float*>(page_error_stats.data_ptr()), num_slots, page_size);
  DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP16(k_input.dtype(), c_type, [&] {
    bsfp8_kv_quant_slots_kernel<c_type, int64_t, THREADS><<<num_slots, THREADS, 0, stream>>>(
        ...
        static_cast<uint8_t*>(k_output.data_ptr()), static_cast<uint8_t*>(v_output.data_ptr()),
        static_cast<uint8_t*>(k_block_scales.data_ptr()),
        static_cast<uint8_t*>(v_block_scales.data_ptr()),
        static_cast<uint8_t*>(page_format.data_ptr()),
        static_cast<float*>(page_error_stats.data_ptr()),
        static_cast<const float*>(routing_thresholds.data_ptr()), num_slots, page_size, num_heads,
        head_dim,
        k_input.stride(0), k_input.stride(1), k_input.stride(2), k_input.stride(3),
        k_output.stride(0), k_output.stride(1), k_output.stride(2), k_output.stride(3),
        k_block_scales.stride(0), k_block_scales.stride(1), k_block_scales.stride(2),
        k_block_scales.stride(3));
    return true;
  });
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(bsfp8_kv_quant_slots, bsfp8_kv_quant_slots);

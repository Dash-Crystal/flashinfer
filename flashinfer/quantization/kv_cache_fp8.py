...
class BlockScaledFP8PagedCache(NamedTuple):
...
@torch.no_grad()
def seal_block_scaled_fp8_pages_cuda(
    k_cache: torch.Tensor,
    v_cache: torch.Tensor,
    slot_mapping: torch.Tensor,
    cache: BlockScaledFP8PagedCache,
) -> None:
    """Seal completed PagedAttention pages into preallocated BSFP8 pools.

    ``k_cache`` and ``v_cache`` use NHD page interiors. A slot at offset zero
    demotes a reused physical page to A16; a slot at the final page offset
    quantizes the now-complete page and promotes its format byte to BSFP8.
    The fused producer records relative RMS and relative maximum reconstruction
    error for each completed page. The page is promoted to BSFP8 only when both
    metrics satisfy ``cache.routing_thresholds``; otherwise it remains A16.
    Both transitions are stream ordered and require no host synchronization.
    """
    ...
    if slot_mapping.dtype != torch.int64 or slot_mapping.ndim != 1:
        raise TypeError("slot_mapping must be a 1D int64 tensor")
    expected_scales = (*k_cache.shape[:-1], k_cache.shape[-1] // 16)
    if cache.k_payload.shape != k_cache.shape or cache.v_payload.shape != v_cache.shape:
        raise ValueError("BSFP8 payload pools must match the A16 cache shapes")
    if (
        cache.k_scales.shape != expected_scales
        or cache.v_scales.shape != expected_scales
    ):
        raise ValueError("BSFP8 scale pools must have head_dim / 16 scale bytes")
    if cache.page_format.shape != (k_cache.shape[0],):
        raise ValueError("page_format must have one byte per physical page")
    if cache.page_error_stats.shape != (k_cache.shape[0], 2):
        raise ValueError("page_error_stats must have shape [num_pages, 2]")
    if cache.page_error_stats.dtype != torch.float32:
        raise TypeError("page_error_stats must use float32")
    if cache.routing_thresholds.shape != (2,):
        raise ValueError("routing_thresholds must contain RMS and maximum limits")
    if cache.routing_thresholds.dtype != torch.float32:
        raise TypeError("routing_thresholds must use float32")
    if (
        cache.page_error_stats.device != k_cache.device
    ...
    get_fp4_kv_quantization_module().bsfp8_kv_quant_slots(
        k_cache,
        v_cache,
        slot_mapping,
        cache.k_global_scale.reshape(1),
        cache.v_global_scale.reshape(1),
        cache.k_payload,
        cache.v_payload,
        cache.k_scales,
        cache.v_scales,
        cache.page_format,
        cache.page_error_stats,

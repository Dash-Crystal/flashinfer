...
Runs the 32 register-expansion cases, the 2 native block-FP8 cases, the 36
...
payloads, maximal block scales and a sub-2^-117 global scale, at q=1 and q=4)
and the 2 independent-reference cases (mixed A16 stream vs stock
...
def main() -> int:
    ...
    # Tails, persistent item boundaries (q=1, the sm90 GMMA host) and extreme E4M3 /
    # scale value ranges (q=1; q=4: the SPEC_DEC mha.cu build).
    for seq_len, nb_ctas, regime, q_len in _mod.TAIL_CASES:
        for mode in ("fp8", "fp4", "mixed"):
            n += 1
            failures += _run(f"[tail-{mode}-{seq_len}-ctas{nb_ctas}-{regime}-q{q_len}]",
                             _mod.test_xqa_mixed_page_transport_tails_and_value_ranges,
                             mode, seq_len, nb_ctas, regime, q_len)

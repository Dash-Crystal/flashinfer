...
- Status 2026-09-04, round 3 "fill / prologue cut" (wt/r3fill @ 6dbc6ae3, `mixed_kv_speed_round3_fill.md` section 10): built, 66/66, all SASS gates pass; locked bench a16 78.8 -> 77.7 (pass), fp8 67.8 -> 67.3 (reject), fp4 60.5 -> 60.4 (reject), mixed 64.4 -> 66.7 (+2.3, reject). Trace: first copy committed as designed (5.31 -> 3.07) but the loader's chunk-0 fill, moved to iteration nbFast, sits behind the converters' start burst (kMetaReady 4.8 -> 9.4 us fp8, 10.9 mixed); the converters' in-loop issue(2) gate then stalls gemm0 2.8-4 us on tile 1, more than the 1.7-1.9 us fill cut; a16's gain is TMA tile ordering only. Trace build hangs in ~5/14 compressed launches (unattributed; production run clean). **Decision: keep nothing**; design (not build) fallback (c) + nbFast = 3 for the compressed modules and a loader-only TMA ordering for a16, after the hang is attributed.
- Round 3 pair attribution (wt/r3pair, `mixed_kv_speed_round3_pair.md`): the slow member is the CTA in physical warp slots 20..39 (396/396 pairs, `%warpid`), every issue-bound segment of every role is 16-28 % longer there while `cp.async` landing is equal (+5 %); a16 (DRAM-bound) shows no split; clusters place their CTAs on distinct SMs with homogeneous slots (no help). Review (section 9) replaced the one-rate wall model by a paired / lone two-rate model: the slow member runs ~10 us ALONE at lone rate after the fast one exits (trace G = 8.8-9.3 us; production 10.6), T_s(paired) ~2.05 us/tile vs T_f 1.409, pair rate 1.20 tiles/us (1.23x lone, not 1.45x); ceiling of any split ~2 us body, base protocol ~1.3 us, net ~0.6-1.0 us after the merger's tag round trip -> predicted fp8 ~67.0 / fp4 ~59.9 / mixed ~63.7 (mixed target 62 unreachable by this lever; dominant lever is [15]/[34]). Built as written (section 10): smid rendezvous with epoch in the merge warp, 8-tile prefix, 2-tile `atom.add.u64` claims one loader iteration apart from the fills (one global round trip per iteration), 6 scratch chunks per static range with sequence tags, tile-count semaphore arrivals (no pool word in the merger, no weights); `XQA_PAIR_DISABLE` = [8] path, `XQA_PAIR_FORCE` = conformance pairing; accept fp8 <= 67.3 / fp4 <= 60.2 / mixed <= 64.0, reject at today's medians or any period > 5 %. First confirmation (section 11.3) rejected by reading: the loader-side claims / fills put ~1.2 us of dependent global latency per tile on the pacing K loader (kl_start->kl_iss 1.51 us vs 0.06; every period +60-100 %). Amendment 2 (section 12, as written in 12.5): a protocol warp (IO warp 2) owns rendezvous, claims, both operands' record fills, Q loads and seams; loaders are per-tile only (frontier read, stage wait, kProg, TMA); fill lead bounded by kProg (C19); 12.6 fixes (short-range deadlock, trace counter warp, static-format fill). **Amendment 2 confirmed and REJECTED (2026-09-04, r3pair @ 441d156f; round-3 doc 12.7-12.8, backends doc dated section):** review fixes first (pair flags read per launch — the paired matrix cases had never executed the pool paths; fp32 reference at 3 bf16 output ulps for paired cases, whose closure point is a race), 76/76 with forced pairing verified live (132 leaders / 132 partners, leaders crossing into active partners' pools; every variant within 1 bf16 ulp of fp32). Bench (locked 5x5, same-session [8] control 78.7 / 68.1 / 60.6 / 65.0): paired a16 92.2 / fp8 85.8 / fp4 80.9 / mixed 105.1; `XQA_PAIR_DISABLE=1` 84.1 / 74.2 / 68.5 / 88.0 — slower than [8] even with the protocol off. Trace with new frontier-wait counters: the loaders wait on the protocol warp's filled frontier 27 % (fp8) / 45 % (fp4) of the body paired, 13-18 % disabled; the serial dependent claim -> fill chain (3-5 us per 2-tile unit under fp8/fp4 memory saturation vs 2.4 us demand) paces the pipeline whatever warp runs it; converter expansions 950-1190 ns/tile (552-742 in [8]); a16 fill +5 us (Q(0) queued behind two fill round trips). Object: 112 B stack, 282/328 B spills (protocol warp per-unit + merge per-item; GEMM / converter / loader paths spill-free), USETMAXREG 2, smem 114.69 KB, occupancy 2/2. Disposition: production stays at [8] (`e113026a` / `039ba5c7`); round 3 stays on record (slot-priority attribution, two-rate model, correct protocol, fp32 paired reference, 32-argument printf finding). Any future pool needs a pipelined producer (claims in flight, fill loads ahead of the previous store, or 16-tile chunk fills with coarse claims) and Q(0) off the fill chain.
...
- Status 2026-09-04 (`mixed_kv_speed_round3_15.md`): **no-go standalone**. Control on the [8] kernel with `XQA_PERSISTENT_CTAS=132` (one 66-tile CTA per SM): fp8 67.7 -> 72.7, fp4 60.4 -> 68.0, mixed 64.2 -> 73.4, a16 79.0 -> 79.5 (locked, interleaved). Lone-CTA tile 0.95 us (fp8) vs pair throughput 0.865 per tile-SM; trace at P=132: unimodal body, GEMM chains 1.17-1.20 us = cadence, converters idle 66 %, no consumer wait on data (K lead 1800-3000 cyc). The layout removes the pair asymmetry but not the consumer chain; keep it as the substrate for [7] (predicted [15]+[7]+fill fp8 ~57.5). a16 body is DRAM-bound at 4.1 TB/s; its lever is the fill, not per-warp TMA issue.
